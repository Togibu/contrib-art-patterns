...
def _generate_border(num_weeks: int, thickness: int) -> list[list[bool]]:
    """
    Outer frame of the grid filled to a given thickness (in cells).
    """
    grid = [[False] * num_weeks for _ in range(7)]
    t = max(1, min(thickness, 3))  # cap so the frame can't fill the whole grid (height 7)
...
def run(context: dict[str, Any]) -> None:
    ...
    grid = _generate_border(num_weeks, thickness)
